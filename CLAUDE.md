...
|---|---:|---|---|
| `INGESTA/Rep_Prod/backend/app/features/consulta_v2/` | **6.009** (5.567 `.py` + 442 YAML) — medido al planificar F4; el 4.901 inicial se quedaba corto un 23 % | `features/consulta/` | F4 |
| `.../features/analisis/api.py` | 2.619 | `features/analisis/` (split por sufijo, no subcarpetas) | F2 |
| `.../features/ingesta/` (`services.py` = 1.940) | 2.153 (medido al portar; el 2.195 de la auditoría inicial incluía un `__init__.py` ya retirado) | `features/ingesta/` | F3 |
| **Test Clas** — repartido, no inventariado hasta 2026-08-20: `consulta_v2/senales.py` (110) + la mitad de `log.py` (~55) + 3 rutas de `api.py` (~34) + `golden/revisar_lote.py` (90) + `cargar_golden_libreta.py` (78) | **~376** `.py` + **404** de `multitab_shell.js:4551-4954` (se reescriben) | `features/consulta/{senales,revision,api_revision}.py` + `features/testclas/` (frontend) | F5 |
...
(**75 casos** medidos, no 34; gate ≥90%) · `cuantificar_golden.yaml` (24 casos) · `analizar_golden.yaml`
...
| DT-8 | 🟡 **Parcialmente verificada (2026-08-20).** El SQL de F2 corre contra dobles en CI, así que ~40 consultas portadas nacieron sin ejercitar contra el esquema real del 139. **Verificado en navegador**: el panel Fundación devuelve datos reales (11 vicepresidencias · 17 gerencias · 18 activos · 62 áreas · 139 campos · 185 fuentes · 151 colisiones, con APIAY/CASTILLA/CUSIANA/CHICHIMENE/RUBIALES como duras), lo que valida la jerarquía y `catalogo_entidades.fuentes_de_activo` — el módulo de mayor riesgo, reimplementado sin la capa conversacional del origen. **Desempeño también verificado** (2026-08-20, tras conectar el `.env` al 139): Global · Mayo 2026 · corte 17/31 con CRUDO 88.857.284 bbl (94,7 %), GAS 72.259.391,14 MSCF (87 %), BLANCOS 618.914 bbl (55,2 %), más la curva diaria y el ritmo mensual — confirma de paso la corrección de unidades de `a6d40ec` contra datos reales. **Siguen sin ejercitar**: el panel Ejecutivo y las 4 pills del acordeón (comportamiento, diferidas, mantenimientos, EBITDA). El ancla **Castilla = 78.629 kUSD** sigue sin comprobar. | F2 (2026-08-20) | Se cierra cuando el usuario recorra el panel Ejecutivo y las 4 pills con datos del 139 (R3) |
| DT-7 | ✅ **Cerrada (2026-08-20)**. `ci.yml` corría `pnpm build`, pero la raíz no tenía ese script: pnpm lo interpretaba como un ejecutable y CI fallaba con `ERR_PNPM_RECURSIVE_EXEC_FIRST_FAIL`. Añadidos `build` y `build:front` a `package.json` (raíz) | Verificación final de F3 (2026-08-20) | Resuelto. El paso de build de CI nunca había llegado a compilar nada desde que se añadió en F2 |
...
| **F1** | Control + Tablas — árbol de reportes + visor (204 líneas, la más chica: valida el patrón end-to-end) | F0 | **Backend completo (2026-08-20)** · 7 endpoints, 99 tests. **Falta el frontend**: `features/control/` solo tiene `mappers/` y `types/`, no hay página ni ruta `/control` |
| **F2** | Análisis — 9 endpoints + EBITDA + diferidas + mantenimientos (2.619 líneas) | F0 | **Código completo (2026-08-20)** · R3 **parcial**: Fundación y Desempeño verificados en navegador contra el 139; faltan el panel Ejecutivo y las 4 pills (DT-8) |
| **F3** | Ingesta — ETL .xlsm, 17 extractores + SSE (2.153 líneas) | F0 | **Completa (2026-08-20)** · verificada contra un `.xlsm` real: 1.103.124 filas, anclas exactas, idempotencia comprobada. R3 pendiente: falta abrir `/ingesta` en navegador |
| **F4** | Consulta — Motor Q v2 completo + panel apilable (**6.009 líneas** medidas, no 4.901) | F2 | **Completa (2026-08-20)** · 16 de 17 bloques; el 17 es la verificación en navegador (R3). Golden: analizar 10/10, cuantificar 24/24, clasificación **61/75 (81 %)** medido **sin Ollama** — 8 casos dependen del modelo, así que el gate del 90 % sigue **sin comprobar** |
| **F5** | Test Clas — laboratorio del clasificador (admin-only) | F4 | **Completa (2026-08-20)** · 3 endpoints admin-only, Controles 2 y 3, pantalla `/test-clas`, CLI de revisión y cargador del golden. Verificado con los 75 casos reales: idempotente, cola limpia, `pct_capa1` = 40 % coincidiendo con el runner. R3 pendiente |
...
| 2026-08-20 (F3) | **Ingesta completa: ETL, 17 extractores, SSE y frontend.** `features/ingesta` (2.153 líneas de origen portadas): `celdas`/`detector`/`transforms` → 17 extractores → 7 loaders de las tablas estrella → repositorio → orquestador → 3 endpoints (`GET /reporte-existente`, `POST /archivo`, `GET /progreso/{id}` en SSE). Frontend `/ingesta`: zona de subida, aviso de reingesta y progreso en vivo por `EventSource`. Nueva dependencia transaccional `get_prod_tx()` (el ETL escribe; `get_prod_db()` sigue siendo la de solo lectura de F1/F2) y un doble de sesión propio (`tests/fakes/db_escritura_falsa.py`) porque el de lectura aborta ante cualquier escritura. **559 tests backend, cobertura 81%**; `pnpm build`, lint y typecheck verdes. **Verificación contra un `.xlsm` real** (`scripts/humo_ingesta.py`, 125 MB, 524 s): 1.103.124 filas escritas, **las dos anclas de paridad de §6 exactas** (`DATOS_MES` = 7.776, `TD_datos_dia` = 5.209) e **idempotencia comprobada** — reingerir el mismo archivo deja conteos idénticos y reutiliza el `reporte_id`. | `Planes/plan_F3_ingesta_2026-08-20.md` (plan v2 auditado) · `prodia_v02_backend/src/features/ingesta/**` · `src/shared/db_prod.py` · `scripts/humo_{extractores,ingesta}.py` · `tests/fakes/{db_escritura_falsa,hoja_sintetica,muestras_xlsm}.py` · `prodia_v02_frontend/src/features/ingesta/**` · `src/app/router.tsx` | **Los tres hallazgos que solo aparecieron ejecutando contra el archivo real** —ninguno lo habría atrapado un test—: (1) **la columna de vicepresidencia se llama distinto según el año** (`GRUPO1_SIGLA`/`NIVEL1_SIGLA`/`VICE`; el archivo de 2024 usa `VICE`) y `vice_id` es NOT NULL, así que reventaba al insertar → `_primera_presente()` en `loaders.py`, con test parametrizado por los tres nombres. (2) **`dim_vicepresidencia` no tiene `nombre` sino `codigo`** — es la única dimensión que rompe el patrón. (3) **La resiliencia "una hoja no tumba el resto" era falsa para errores de BD**: PostgreSQL deja la transacción abortada y el propio `INSERT` en la bitácora del manejador de error reventaba con `InFailedSqlTransaction`, enmascarando la causa real; ahora `SQLAlchemyError` se propaga (revierte y el evento final dice `revertido`) y solo los fallos de lectura permiten continuar. Además, sobre el spike del Bloque 0: se midió que `BaseHTTPMiddleware` **no** bufferiza el streaming (eventos a 0,00/1,00/2,00/3,02/4,02 s a través del proxy de Vite), lo que cerró DA-7 antes de escribir nada. Y dos trampas de los dobles de test que habrían dado verde en falso: guardar la *referencia* al buffer de parámetros (los loaders lo vacían con `.clear()`) y devolver una lista donde openpyxl devuelve un generador (los loaders consumen la cabecera con `next()`). **Pendiente R3**: falta abrir `/ingesta` en navegador y subir un archivo. |
| 2026-08-20 (F5) | **Test Clas completo: los Controles 2 y 3 del ciclo de crecimiento.** Backend: `senales.py` (Control 2), `revision.py` (la cola), `libreta.py` ampliado con el resumen de KPIs, y `api_revision.py` — 3 endpoints **admin-only en router aparte**, para que `require_admin` vaya una vez en el constructor y no endpoint por endpoint. Scripts: `revisar_lote.py` (CLI) y `cargar_golden_libreta.py`. Frontend: `/test-clas` con chat de prueba, libreta tabulada, teclado y `RutaAdmin`. **Verificado contra la libreta real**: los 75 casos del golden cargan, la 2ª pasada deja 75 (idempotente), la cola de pendientes queda en 0 y `pct_capa1` = 40 % **coincide con el runner del golden** (30/75) — dos caminos independientes dando el mismo número. **878 tests backend** (línea base 819) y **333 frontend** (era 291), cobertura 84 % en ambos. | `Planes/plan_F5_test_clas_2026-08-20.md` (plan v4, auditado ejecutando los pipelines) · `prodia_v02_backend/src/features/consulta/{senales,revision,libreta,api_revision,schemas_revision}.py` · `src/shared/auth_guards.py` · `src/middleware/auth.py` · `src/core/exceptions.py` · `scripts/{revisar_lote,cargar_golden_libreta}.py` · `prodia_v02_frontend/src/features/testclas/**` · `src/shared/components/RutaAdmin/**` | **`require_admin` estaba roto desde F0 y nadie lo sabía**, porque ningún endpoint lo usaba: existía desde el cimiento sin ejercitarse. Al montarlo, un administrador legítimo recibía **503**. La causa tardó en verse porque el síntoma apuntaba al sitio equivocado: `AuthMiddleware` deja el `User` en `request.state` y cierra su sesión, y cualquier commit posterior **expira los atributos**; leer `user.is_admin` —una columna corriente— dispara un refresco contra una sesión muerta, lanza `DetachedInstanceError` y el manejador global lo traduce a "la base de datos no está disponible". No había ningún problema de base de datos. Ahora el middleware resuelve el criterio con la sesión viva y deja **valores planos** (`es_admin`, `user_id`, `group_id`); un bool no caduca. `get_allowed_campos` tenía el mismo defecto latente. Segundo hallazgo, del mismo tirón: **el engine de tests eran varias bases vacías** — `sqlite:///:memory:` sin `StaticPool` abre una BD nueva por conexión, y no se notaba porque los tests existentes reutilizan una sola sesión; se destapó con el primer endpoint que resuelve la suya por `Depends`. Y un tercero que encontró un test **contra código que acababa de escribir**: el efecto de arranque dependía de `escanear`, que TanStack recrea en cada render, así que el escaneo se redisparaba solo —reintroduciendo el derroche que H5 corrige— y el chat además lo llamaba tras cada pregunta, cuando una clasificación recién hecha no puede tener señales todavía. |
...
| 2026-08-20 (navegación · unidades · plan F4) | **Tres correcciones y el plan de F4.** (1) **Navegación**: ni `/analisis` (F2) ni `/ingesta` (F3) estaban enlazadas en el header — las páginas existían pero solo se llegaba escribiendo la URL. Se añade un `<nav>` con las 3 secciones y **un test que recorre las rutas y exige que cada una tenga su enlace**, para que la próxima fase que lo olvide rompa el build en vez de entregar una pantalla inalcanzable. (2) **Unidades (A5, 3ª aparición)**: CRUDO salía `88.857.284` sin unidad mientras GAS sí decía MSCF; y en `PanelEjecutivo` el gas salía duplicado (`… MSCF MSCF/mes`) porque el backend ya manda `unidad`. Nace `formatBbl` (y `formatBl` queda como separador de miles, porque `PanelFundacion` lo usa para contar días y campos, donde «139 bbl» sería peor) más `formatRitmoDiario` (BOPD/MSCFD: el ritmo es una TASA, no un volumen). (3) **`.env`**: apuntaba a `localhost` —correcto para F3, que escribe— y por eso F2 daba 503 contra el 139; se documentan los dos entornos en `.env.example` con la trampa del `£` (`%C2%A3`) de `OPS_DATABASE_URL`. **Plan de F4 entregado** (v2, auditado contra el origen y contra los pipelines): 8 hallazgos AP-1…AP-8, DA-1/2/3 cerradas. | `prodia_v02_frontend/src/app/layouts/**` · `src/shared/utils/format.ts` · `src/features/analisis/components/{PanelDesempeno,PanelEjecutivo,AcordeonFoco}/**` · `prodia_v02_backend/{.env.example,pyproject.toml}` · `Planes/plan_F4_consulta_2026-08-20.md` | **El hallazgo con más consecuencias es AP-1, y se comprobó solo**: el gate de cobertura es GLOBAL (75 %) y F4 añadirá ~4.500 líneas de golpe; mientras se auditaba, F3 hizo caer el total de 78,97 % a **72,41 %** —CI rojo para todo el equipo por código de otra fase— y volvió a 81,51 % al cubrir sus extractores. Por eso el plan v2 verifica cobertura **al cerrar cada bloque**, no al final. Segundo hallazgo: **`cuant_kpi`, el tipo de panel más común, no existe en el dispatcher del sistema viejo** (`grep -c` → 0) y cae al fallback por accidente — es Q5 con prueba, y de ahí la unión discriminada con `never` en F4. Tercero: el hook `gen-types-check` se dispara con TODO fichero de `src/features/**`, así que en F4 el CERO I/O al importar deja de ser buena práctica y pasa a bloqueante. |
